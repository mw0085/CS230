...
if(isset($_POST['signup-submit'])){
    ...
    $username = $_POST['uname']; #variable grabs name from signup.php
    $email = $_POST['email'];
    $passw = $_POST['pwd'];
    ...
    if($passw !== $passw_rep){ #if passwords don't match
        ...
        $sql = "SELECT uname FROM users WHERE uname=?"; #checks to see if sql injection or username is repeated
        $stmt = mysqli_stmt_init($conn);
        if(!mysqli_stmt_prepare($stmt, $sql)){ #sqlInjection
